...
function handlePaymentProcess(e) {
    // עצירה מוחלטת של רענון הדף
    if (e) {
        e.preventDefault();
        e.stopPropagation();
    }
...
// חיבור לכפתור - רק אם הוא נמצא בדף
document.addEventListener("DOMContentLoaded", function () {
    const payBtn = document.getElementById("payNowBtn");
    if (payBtn) {
        // מסירים כל חיבור קודם ומחברים מחדש
        payBtn.onclick = handlePaymentProcess;
    }
});
// הוספת מאזין אירועים בצורה ישירה ובלתי תלויה
document.body.addEventListener('click', function(event) {
    // בדיקה אם האלמנט שנלחץ הוא כפתור התשלום
    if (event.target && event.target.id === 'payNowBtn') {
        event.preventDefault();
        event.stopPropagation();
        
        console.log("הכפתור זוהה!");
        alert("Payment completed successfully!");
        window.location.href = 'p2.html';
    }
});